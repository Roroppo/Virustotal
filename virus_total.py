...
class Apikey:
	def __init__(self):
		...
		self.cursor.execute('use virustotal')
...
class Analysis:
	...
	def virustotal(self):
		...
		self.url = str(self.cursor.execute('select analyzed_url from analysis_results where id = %d'% (self.analysis_id)))
		#現在の時間
	...
	def __calc(self):
		...
		self.cursor.execute('update analysis_results set analyzed_time =now(), analysis_result = %d, updated_at = now()'% (decjson["positives"]))
