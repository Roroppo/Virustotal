...
class Apikey:
	def __init__(self):
		#database接続
		self.connect = mysql.connector.connect(user='root', password='Masasi.1109', host='root', database='virustotal.sql', charset='utf8')
	...
	def set_apikeys(self):
		apikeys = ["c2d51ce4334a95e2e44737bd2d8314622f13f64476897511a4df73cb97a20ae5",
					...
					"3017f79daf07d3b9efda79d246fbc6563a6a30f02fe8e2cce102ae0c9b36ee8b",
...
class Analysis:
	...
	def virustotal(self,crawler_id,url_id):
		self.crawler_id = crawler_id
		self.url_id = int(url_id)	
		self.url = self.corsor.execute('select short_url from short_urls where id = %d',self.url_id)
		#現在の時間
		now = datetime.datetime.now()
		#idの最大値をVirustotal_api_keysから取得
		for m in range(mMax = int(self.consor.execute('select Max(id) from virustotal_api_keys'))):
			#now.minuteとuse_timeが一緒
			if str(now.minute) == str(self.consor.execute('select use_time from virustotal_api_keys where id = %d',m+1) ):
				#この分に使った回数が4回か
				if int(self.consor.execute('select use_numbers from virustotal_api_keys where id = %d ',m+1)) == 4:
					#今見ているidが最大値を満たしているか
					if m + 1 == mMax: 
						time.sleep(60)
						#sleepしたらidの探索を元に戻す
				...
				self.consor.execute('update virustotal_api_keys set use_numbers = 0 where id = %d',m+1)
				break	 	

		self.apikey = str(self.consor.execute('select api_key from virustotal_api_keys where id = %d',m+1))			
		analysis_id = self.__calc()	
		#table virustotal_api_keysのuse_time,use_numbersにnow,use_number+1をして格納
		self.consor.execute('update virustotal_api_keys set use_time = now(),use_numbers += 1 where id = %d',m+1)
		return analysis_id
	def __calc(self):
		...
		#apikeys = ["e772903ca2c5a11ea3a03382874774479871347e913981a4a29415249398416a",
					#"c2d51ce4334a95e2e44737bd2d8314622f13f64476897511a4df73cb97a20ae5",
					#"e395da8c3345f48e380d008cd41e822efd696938dd528e337d7f10cdde1fbfa9",
					#"6c791140b269f77742420ff75583229768364cef5e8e6ce903786e7d88270d1a"]
		#keys = 4
		#time.sleep(15/keys)
